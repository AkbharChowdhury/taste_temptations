...
export class RecipeUI {
    ...
    intolerance () {
        return sortedArray(intolerances).map(intoleranceHtml).join().replaceAll(',', '');
    }
    cuisines (){
        return sortedArray(cuisines).map(cuisinesHtml).join().replaceAll(',', '');
    }
    meals() {
    ...
    number(numItems=3, incrementBy=10) {
